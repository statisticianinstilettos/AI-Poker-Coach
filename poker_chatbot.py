...
from database import save_chat, get_user_chats, save_tournament_result, get_user_tournament_results, get_user_stats
...
if authentication_status:
    ...
    user_stats = get_user_stats(config['credentials']['usernames'][username]['email'])
    if user_stats:
    ...
    def process_message():
        if st.session_state.user_message and st.session_state.user_message != st.session_state.last_processed_input:
            ...
            if len(st.session_state.chat) == 0:
                ...
                user_tournaments = get_user_tournament_results(config['credentials']['usernames'][username]['email'])
                tournament_context = ""
                if user_tournaments:
                ...
                past_chats = get_user_chats(config['credentials']['usernames'][username]['email'])
                chat_context = ""
                if past_chats:
                ...
                try:
                    ...
                    if submitted:
                        if not venue:
                            ...
                            try:
                                save_tournament_result(config['credentials']['usernames'][username]['email'], tournament_data)
